...
Montar uma lista de leads qualificados (ICP) a partir de scraped do LinkedIn. O fluxo busca perfis que se encaixam em critérios e grava os dados na base.
...
**O que faz:**
- Cria o campo `random_numb` com valor: `Math.floor(Math.random() * 10)`
...
**Por que existe:** Esses filtros servem para evitar o padrão repetitivo de busca de leads.
...
#### O que é Apify? O que é Ator? O que é o ID na requisição?
...
- Se não estiver vazio (ou seja, se existir), continua o fluxo.
- Se não, não faz nada.
...
**Headers `Prefer` e `resolution`:**

Além dos parâmetros `apikey`, `Authorization` e `Content-Type`, temos também o `Prefer` com:

| Valor | Função |
|-------|--------|
| `return=representation` | Retorna os registros criados ou atualizados no corpo da resposta. |
| `resolution=merge-duplicates` | Define o que fazer em conflito de chave única: em vez de erro, atualiza o registro existente (merge). |
...
Busca apenas as empresas que vieram do scrap anterior. Os detalhes do node são os mesmos do scrap de perfis; a diferença é que agora é filtrado por empresa.

---

### Update a row1

- Atualiza os dados colhidos com o filtro da URL do LinkedIn.
- Sem risco de atualizar registros errados.

---

## Resumo: o que acontece em cada execução
...
- Os headers `Prefer` e `resolution` — é isso mesmo que coloquei lá em cima? Dei uma pesquisada aqui.
- Por que no Upsert Supabase usamos a chamada HTTP Request da tabela e não usamos o node do Supabase direto?
...
1. **Credenciais expostas:** API keys do Supabase e Apify estão no workflow. Ideal: usar credenciais do n8n ou variáveis de ambiente.

2. **Duplicidade de "URL da Empresa" no body:** O body tem duas entradas para `URL da Empresa`:
   - Primeiro campo: `{{ $json.currentPosition[0]?.companyLinkedinUrl ?? null }}`
   - Segundo campo: `{{ $json.companyWebsites[0]?.url ?? null }}`
   
   A segunda sobrescreve a primeira. Vale definir regra clara (priorizar website ou LinkedIn).
